...
1. **`backend/` + `frontend/`** 한 세트로 CORS + React + API 연동 완료  
2. `backend/`에 **CORS** 설정 (`http://localhost:3000` 허용)  
3. `frontend/`에서 **목록 조회(GET)** + **추가(POST)**  
...
## ⚠️ 폴더 구조 — `backend` / `frontend` 로 구분

로컬 작업·제출 모두 아래 **형제 폴더** 구조를 사용합니다.

```
{작업폴더}/                  # 예: week06-todo (이름은 자유)
├── backend/                 # 5주차 Spring Boot 복사본
│   ├── pom.xml
│   ├── mvnw
...
**제출 경로** (`submissions/본인아이디/` 아래도 동일):
...
- `backend/`와 `frontend/` **둘 다** 제출합니다.  
- Spring Boot를 `frontend/` 안에 넣거나, React를 `backend/src` 안에 넣지 **않습니다.**

---

## 0. 사전 준비 — `backend` 폴더

5주차 **Spring Boot Todo API** 프로젝트를 **`backend/`** 폴더에 둡니다.
...
```bash
cd backend           # 5주차 Spring Boot 프로젝트
...
| 3 | `frontend/` 폴더에 React 생성 | `npm start`로 화면 열림 |
...
- `backend/src/.../CorsConfig.java`  
- `frontend/src/api/axios.js`  
- `submissions/아이디/README.md`에 **backend / frontend** 실행 명령
...
`submissions/아이디/README.md`에 **backend / frontend** 기준으로 적습니다.
...
- `submissions/아이디/` 아래 **`backend/`**, **`frontend/`** 폴더를 **반드시 둘 다** 제출  
